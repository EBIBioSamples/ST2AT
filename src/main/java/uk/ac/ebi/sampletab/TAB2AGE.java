...
public class TAB2AGE {
    ...
    static Set<String> attributeTypes = new HashSet<String>(Arrays.asList("Characteristics", "Provider", "Material"));
    ...
    public static void convert(String path) throws IOException {
        ...
        while ((str = in.readLine()) != null) { // first pass through the file - find if there are groups defined; if not, a
        ...
        while ((str = in.readLine()) != null) {
            if (str.startsWith("[SCD]"))
                lastBlock = true;
            else if (str.startsWith("#") || str.startsWith("[") || str.contains("[MSI]")
                    || str.trim().compareTo("") == 0)
                continue;
            ...
            if (oldCat.compareTo(newCat) != 0) { // MSI section processed in blocks, e.g. "Submission" block and
                                                 // "Organization" block
                    ...
                    if (oldCat.compareTo("Submission") == 0 || oldCat.compareTo("Term Source") == 0
                        ...
                        for (int j = 1; j < blockColNum; j++) {
                            ...
                            for (int i = 0; i < v.size(); i++) {
                                ...
                                if (i == 0
                                        && (oldCat.compareTo("Term Source") == 0 || oldCat.compareTo("Database") == 0))
                                    token = "$" + token;
                                out.write(token + "\t");
                            }
                            out.write("\n");
        ...
        while ((str = in.readLine()) != null) {
            ...
            for (int curColIdx = 0; curColIdx < strArr.length; curColIdx++) {
                ...
                if (nodeTypes.contains(header)) {
                    ...
                    curUnitIdx = -1;
                    curAttIdx = -1;
                } else if (attributeTypes.contains(header)
                        || curAttIdx == -1
                        && (header.compareTo("Term Source REF") == 0 || header.compareTo("Comment") == 0)
                        || (header.compareTo("Comment") == 0 && !attributesCommentsAllowed
                                .contains(colDefs[curAttIdx].attClass))) {
        ...
        in.close();
        for (int pivotColIdx = 0; pivotColIdx < colNumber; pivotColIdx++) { // reads file as many times as there are "pivot" columns in SCD (i.e.,
                                                                            // defining objects)
            ...
            while ((str = in.readLine()) != null) {
                ...
                for (int curColIdx = pivotColIdx + 1; curColIdx < strArr.length; curColIdx++) {
                    if (colDefs[curColIdx].linkedColumnNumber == pivotColIdx
                            || colDefs[curColIdx].linkedColumnNumber == curAttIdx
                            || colDefs[curColIdx].linkedColumnNumber == curUnitIdx) {
        ...
        while ((str = in.readLine()) != null) {
            ...
            if (oldCat.compareTo(newCat) != 0) {
                ...
                if (oldCat.compareTo("") != 0
                        && blockColNum > 1
                        && (oldCat.compareTo("Publication") == 0 || oldCat.compareTo("Organization") == 0 || oldCat
                                .compareTo("Person") == 0)) {
                    out.write(oldCat + "\t");
                    for (int i = 0; i < v.size(); i++)
