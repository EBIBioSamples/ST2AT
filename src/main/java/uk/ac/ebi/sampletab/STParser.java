            ...
            if (!sampleSection) {
                ...
                if (headerLine == null) {
                    ...
                    if (!headerLine.get(0).equals(SAMPLENAME))
                        throw new STParseException("The first column should be " + SAMPLENAME + " Line: "
                                + reader.getLineNumber());
                } else {
                    if (parts.size() > headerLine.size())
                        throw new STParseException("Some values are beyond the annotation. Line: "
                                + reader.getLineNumber());
                    ...
                    for (int i = 0; i < runlen; i++) {
                        ...
                        if (SAMPLENAME.equals(hdr)) {
                            ...
                            if (group != null) {
                                if (group.getValue() == null)
                                    throw new STParseException("Group has no accession. Line: "
                                            + reader.getLineNumber());
                                ...
                                if (sample.getValue() == null)
                                    throw new STParseException("Sample has no accession. Line: "
                                            + reader.getLineNumber());
                            ...
                            sample.addAnnotation(SAMPLENAME_AGE,
                                    attribute = new Attribute(SAMPLENAME_AGE, parts.get(i)));
                        }
                        if (GROUPNAME.equals(hdr)) {
                            ...
                            if (!found)
                                throw new STParseException("No such term source defined: '" + tsRef + "'. Line: "
                                        + reader.getLineNumber());
